...
import { extractParallelNarration } from '@/lib/narration'
...
export function ParallelReaderClient({ novelId, novelTitle, chapterNum, availableChapters, lines }: Props) {
  const { fontSize, fontFamily, languageMode } = useSettings()

  const prevNum = chapterNum > 1 ? chapterNum - 1 : null
  const nextNum = chapterNum < availableChapters ? chapterNum + 1 : null
  const narrationSegments = extractParallelNarration(lines, languageMode)
  const narrationLang = languageMode === 'de' ? 'de-DE' : 'en-GB'
