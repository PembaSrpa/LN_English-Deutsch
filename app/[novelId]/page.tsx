    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-8">
        <div className="flex gap-4 mb-6">
          <div className="w-20 flex-shrink-0 rounded-xl overflow-hidden border border-neutral-300 dark:border-neutral-700 bg-neutral-200 dark:bg-neutral-800" style={{ aspectRatio: '2/3' }}>
            {novel.coverImage
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-1 mb-3">
              {novel.genre.map(g => <span key={g} className="text-[9px] border border-neutral-300 dark:border-neutral-600 rounded-md px-1.5 py-0.5 text-neutral-400 dark:text-neutral-500">{g}</span>)}
            </div>
        ...
        {chapters.length > 0 && (
          <>
            ...
            <div className="flex flex-col divide-y divide-neutral-200 dark:divide-neutral-700 border border-neutral-300 dark:border-neutral-700 rounded-xl overflow-hidden">
              {chapters.map(ch => (
                <Link key={ch.id} href={`/${novelId}/${ch.id}`}
                  className="flex items-center gap-3 py-3 px-4 hover:bg-neutral-200 dark:hover:bg-neutral-700 transition-colors group">
                  <span className="text-[10px] text-neutral-400 dark:text-neutral-500 w-5 text-right flex-shrink-0 tabular-nums">{ch.id}</span>
                  ...
                  {ch.germanPercent && <span className="text-[10px] text-neutral-400 dark:text-neutral-500 flex-shrink-0">{ch.germanPercent}</span>}
                  <IconArrowLeft size={11} className="text-neutral-300 dark:text-neutral-600 rotate-180 flex-shrink-0" />
                </Link>
              ))}
            </div>
          </>
