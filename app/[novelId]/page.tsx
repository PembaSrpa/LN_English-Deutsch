    ...
    <div className="min-h-screen flex flex-col bg-neutral-200 dark:bg-neutral-900">
      ...
      <main className="flex-1 px-8 py-8">
        <div className="flex gap-4 mb-6">
          <div className="w-16 flex-shrink-0 flex items-center justify-center border border-neutral-300 dark:border-neutral-700 rounded-xl bg-neutral-300 dark:bg-neutral-800" style={{ height: '88px' }}>
            {novel.coverImage
              ? <img src={novel.coverImage} alt={novel.title} className="w-full h-full object-cover rounded-xl" />
              : <span className="text-2xl font-bold text-neutral-400 dark:text-neutral-600">{novel.title.slice(0, 1)}</span>
            }
          </div>
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-neutral-500 dark:text-neutral-400 leading-relaxed line-clamp-3">{novel.description}</p>
          </div>
        </div>

        {novel.type === 'pdf' ? (
          <Link href={`/${novelId}/read`} className="block w-full py-2.5 text-center text-xs font-bold bg-neutral-800 dark:bg-neutral-100 text-white dark:text-black rounded-full hover:opacity-90 transition-opacity mb-6">
            Open PDF Reader
          </Link>
        ) : (
        ...
        {novel.type === 'md' && (
          <>
            <TutorialHint
              ...
              body="German density increases chapter by chapter. Start from Ch. 1."
            />
            ...
            <div className="flex flex-col divide-y divide-neutral-300 dark:divide-neutral-700 border border-neutral-300 dark:border-neutral-700 rounded-xl overflow-hidden">
              {chapters.map(ch => (
                <Link key={ch.id} href={`/${novelId}/${ch.id}`}
                  className="flex items-center gap-3 py-3 px-4 hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-colors group"
                >
