# 🇩🇪 Goethe A1 German — Complete Reference Guide
> Official vocabulary from the Goethe-Institut A1 (Start Deutsch 1) word list, organized with grammar tables and categorized glossaries.

---

# PART 1 — GRAMMAR TABLES

---

## 1. The German Articles (Definite & Indefinite)

### 1a. Definite Articles — "the" (der / die / das)

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominativ (subject) | der | die | das | die |
| Akkusativ (direct object) | den | die | das | die |
| Dativ (indirect object) | dem | der | dem | den |
| Genitiv (possession) | des | der | des | der |

>  Only Masculine changes in Akkusativ: der → den
>  Dativ plural always adds -n to the noun: *den Kindern, den Männern*

---

### 1b. Indefinite Articles — "a/an" (ein / eine)

| Case | Masculine | Feminine | Neuter |
|------|-----------|----------|--------|
| Nominativ | ein | eine | ein |
| Akkusativ | einen | eine | ein |
| Dativ | einem | einer | einem |

>  No plural form for *ein*. Use just the noun (or *keine* for "no/not any").

---

### 1c. Negative Article — "no / not a" (kein / keine)

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominativ | kein | keine | kein | keine |
| Akkusativ | keinen | keine | kein | keine |
| Dativ | keinem | keiner | keinem | keinen |

---

## 2. Possessive Articles (mein, dein, sein, ihr …)

All possessives follow the same endings as kein/ein:

| Pronoun | Meaning | Nominativ M | Nominativ F | Nominativ N | Nominativ Pl |
|---------|---------|-------------|-------------|-------------|--------------|
| mein- | my | mein | meine | mein | meine |
| dein- | your (informal) | dein | deine | dein | deine |
| sein- | his / its | sein | seine | sein | seine |
| ihr- | her / their | ihr | ihre | ihr | ihre |
| unser- | our | unser | unsere | unser | unsere |
| euer- | your (plural) | euer | eure | euer | eure |
| Ihr- | your (formal) | Ihr | Ihre | Ihr | Ihre |

### Full declension of mein (example for all possessives):

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominativ | mein | meine | mein | meine |
| Akkusativ | meinen | meine | mein | meine |
| Dativ | meinem | meiner | meinem | meinen |

---

## 3. Nominativ vs. Akkusativ vs. Dativ — When to Use Which

| Case | Question | Use | Example |
|------|----------|-----|---------|
| Nominativ | Wer? / Was? | Subject (who/what does the action) | Der Mann kommt. |
| Akkusativ | Wen? / Was? | Direct object (what is affected) | Ich sehe den Mann. |
| Dativ | Wem? | Indirect object (to/for whom) | Ich gebe dem Mann das Buch. |

---

## 4. Prepositions + Their Cases

### 4a. Always Akkusativ

| Preposition | Meaning | Example |
|-------------|---------|---------|
| durch | through | Gehen Sie durch die Tür. |
| für | for | Das ist für dich. |
| gegen | against / around | Fahr nicht gegen den Baum! |
| ohne | without | Ohne Geld geht das nicht. |
| um | around / at (time) | Er kommt um sieben Uhr. |
| bis | until / up to | Ich fahre bis Stuttgart. |

---

### 4b. Always Dativ

| Preposition | Meaning | Example |
|-------------|---------|---------|
| aus | from / out of | Er kommt aus Brasilien. |
| bei | at / near / with | Ich wohne bei meinen Eltern. |
| mit | with | Ich fahre mit dem Bus. |
| nach | after / to (cities/countries) | Ich fliege nach Berlin. |
| seit | since / for | Ich wohne seit drei Jahren hier. |
| von | from / of / by | Das Auto von Felix ist kaputt. |
| zu | to / at | Ich gehe zu Fuß. |
| gegenüber | opposite | Die Post ist gegenüber dem Bahnhof. |

---

### 4c. Two-Way Prepositions (Wechselpräpositionen) — Akkusativ OR Dativ

> Rule: Movement/direction → Akkusativ (Wohin?) | Location/state → Dativ (Wo?)

| Preposition | Meaning | Akkusativ (movement) | Dativ (location) |
|-------------|---------|----------------------|------------------|
| an | at / on (vertical) | Stell es an die Wand. | Es hängt an der Wand. |
| auf | on (horizontal) | Leg es auf den Tisch. | Es liegt auf dem Tisch. |
| hinter | behind | Stell es hinter den Schrank. | Es steht hinter dem Schrank. |
| in | in / into | Geh in die Küche. | Er ist in der Küche. |
| neben | next to | Stell es neben den Stuhl. | Es steht neben dem Stuhl. |
| über | above / over | Häng es über das Sofa. | Es hängt über dem Sofa. |
| unter | under / below | Stell es unter den Tisch. | Es steht unter dem Tisch. |
| vor | in front of | Stell es vor die Tür. | Es steht vor der Tür. |
| zwischen | between | Stell es zwischen die Stühle. | Es steht zwischen den Stühlen. |

---

## 5. Personal Pronouns

| Person | Nominativ | Akkusativ | Dativ |
|--------|-----------|-----------|-------|
| I | ich | mich | mir |
| you (informal) | du | dich | dir |
| he | er | ihn | ihm |
| she | sie | sie | ihr |
| it | es | es | ihm |
| we | wir | uns | uns |
| you (plural) | ihr | euch | euch |
| they | sie | sie | ihnen |
| you (formal) | Sie | Sie | Ihnen |

---

## 6. Verb Conjugation — Present Tense (Präsens)

### 6a. Regular Verbs — Pattern with machen (to do/make)

| Person | Ending | machen | arbeiten | heißen |
|--------|--------|--------|----------|--------|
| ich | -e | mache | arbeite | heiße |
| du | -st | machst | arbeitest | heißt |
| er/sie/es | -t | macht | arbeitet | heißt |
| wir | -en | machen | arbeiten | heißen |
| ihr | -t | macht | arbeitet | heißt |
| sie/Sie | -en | machen | arbeiten | heißen |

>  Verbs ending in -ten / -den (arbeiten, reden) add an extra -e before -st and -t.

---

### 6b. Irregular Verbs — Core A1 Verbs

| Infinitiv | ich | du | er/sie/es | wir | ihr | sie/Sie | Meaning |
|-----------|-----|----|-----------|-----|-----|---------|---------|
| sein | bin | bist | ist | sind | seid | sind | to be |
| haben | habe | hast | hat | haben | habt | haben | to have |
| werden | werde | wirst | wird | werden | werdet | werden | to become |
| fahren | fahre | fährst | fährt | fahren | fahrt | fahren | to drive/go |
| laufen | laufe | läufst | läuft | laufen | lauft | laufen | to run/walk |
| schlafen | schlafe | schläfst | schläft | schlafen | schlaft | schlafen | to sleep |
| lesen | lese | liest | liest | lesen | lest | lesen | to read |
| sehen | sehe | siehst | sieht | sehen | seht | sehen | to see |
| essen | esse | isst | isst | essen | esst | essen | to eat |
| geben | gebe | gibst | gibt | geben | gebt | geben | to give |
| sprechen | spreche | sprichst | spricht | sprechen | sprecht | sprechen | to speak |
| helfen | helfe | hilfst | hilft | helfen | helft | helfen | to help |
| nehmen | nehme | nimmst | nimmt | nehmen | nehmt | nehmen | to take |
| anfangen | fange an | fängst an | fängt an | fangen an | fangt an | fangen an | to begin |
| laufen | laufe | läufst | läuft | laufen | lauft | laufen | to walk/run |
| waschen | wasche | wäschst | wäscht | waschen | wascht | waschen | to wash |
| tragen | trage | trägst | trägt | tragen | tragt | tragen | to carry/wear |
| wissen | weiß | weißt | weiß | wissen | wisst | wissen | to know (fact) |

---

## 7. Modal Verbs (Modalverben)

Modal verbs express ability, permission, obligation, or desire. They are always used with an infinitive at the end.

| Infinitiv | ich | du | er/sie/es | wir | ihr | sie/Sie | Core Meaning |
|-----------|-----|----|-----------|-----|-----|---------|--------------|
| können | kann | kannst | kann | können | könnt | können | can / to be able to |
| müssen | muss | musst | muss | müssen | müsst | müssen | must / have to |
| wollen | will | willst | will | wollen | wollt | wollen | want to |
| sollen | soll | sollst | soll | sollen | sollt | sollen | should / supposed to |
| dürfen | darf | darfst | darf | dürfen | dürft | dürfen | may / allowed to |
| mögen | mag | magst | mag | mögen | mögt | mögen | to like |
| möchten | möchte | möchtest | möchte | möchten | möchtet | möchten | would like to |

### Examples:
| German | English |
|--------|---------|
| Ich kann Deutsch sprechen. | I can speak German. |
| Du musst jetzt gehen. | You have to go now. |
| Er will Arzt werden. | He wants to become a doctor. |
| Hier darf man nicht rauchen. | You may not smoke here. |
| Was möchten Sie trinken? | What would you like to drink? |

---

## 8. Separable Verbs (Trennbare Verben)

The prefix splits off and goes to the end of the sentence in present tense.

| Verb | Meaning | Present Tense Example |
|------|---------|----------------------|
| an·rufen | to call (phone) | Ich rufe dich an. |
| ab·fahren | to depart | Der Zug fährt um 8 Uhr ab. |
| ab·holen | to pick up | Ich hole dich ab. |
| an·fangen | to begin | Der Kurs fängt um 9 Uhr an. |
| auf·stehen | to get up | Ich stehe um 6 Uhr auf. |
| aus·steigen | to get off | Wo steigst du aus? |
| ein·kaufen | to shop | Ich kaufe heute ein. |
| ein·laden | to invite | Darf ich dich einladen? ← *NOT separable in infinitive* |
| an·machen | to turn on | Mach das Licht an! |
| aus·machen | to turn off | Mach das Licht aus! |
| mit·kommen | to come along | Kommst du mit? |
| fern·sehen | to watch TV | Wir sehen heute Abend fern. |
| aus·füllen | to fill out | Füllen Sie das Formular aus! |

---

## 9. Perfect Tense (Perfekt) — Talking About the Past

Structure: haben/sein (conjugated) + Partizip II (at the end)

### 9a. Partizip II Formation

| Type | Rule | Example |
|------|------|---------|
| Regular | ge- + stem + -t | machen → gemacht |
| Regular (-ieren) | no ge- + -t | telefonieren → telefoniert |
| Separable | prefix + ge- + stem + -t | anrufen → angerufen |
| Irregular | ge- + changed stem + -en | fahren → gefahren |

### 9b. haben vs. sein

sein is used with verbs of movement/change of state:

| Verb | Partizip II | Example |
|------|-------------|---------|
| gehen | gegangen | Ich bin gegangen. |
| fahren | gefahren | Er ist gefahren. |
| kommen | gekommen | Sie ist gekommen. |
| fliegen | geflogen | Wir sind geflogen. |
| aufstehen | aufgestanden | Ich bin aufgestanden. |
| einsteigen | eingestiegen | Er ist eingestiegen. |
| aussteigen | ausgestiegen | Ich bin ausgestiegen. |
| bleiben | geblieben | Sie ist geblieben. |
| werden | geworden | Er ist Arzt geworden. |
| sein | gewesen | Wo bist du gewesen? |

haben is used with most other verbs:

| Verb | Partizip II | Example |
|------|-------------|---------|
| machen | gemacht | Ich habe das gemacht. |
| kaufen | gekauft | Er hat ein Auto gekauft. |
| essen | gegessen | Wir haben gegessen. |
| trinken | getrunken | Sie hat Kaffee getrunken. |
| sehen | gesehen | Ich habe ihn gesehen. |
| sprechen | gesprochen | Wir haben gesprochen. |
| schreiben | geschrieben | Er hat eine E-Mail geschrieben. |

---

## 10. Sentence Structure (Wortstellung)

### 10a. Basic Word Order

| Position 1 | Position 2 (Verb) | Middle | End |
|------------|-------------------|--------|-----|
| Ich | komme | morgen | — |
| Morgen | komme | ich | — |
| Ich | möchte | ein Bier | trinken |
| Er | hat | das Buch | gelesen |

>  The verb always stays in position 2 in a main clause, no matter what comes first!

### 10b. Questions

| Type | Structure | Example |
|------|-----------|---------|
| Yes/No question | Verb first | Kommst du mit? |
| W-question | W-word + Verb | Woher kommen Sie? |

### 10c. Common W-Question Words

| German | English | Example |
|--------|---------|---------|
| wer | who | Wer ist das? |
| was | what | Was machst du? |
| wo | where | Wo wohnst du? |
| woher | where from | Woher kommen Sie? |
| wohin | where to | Wohin fährst du? |
| wann | when | Wann bist du fertig? |
| warum | why | Warum kommst du nicht? |
| wie | how | Wie geht's? |
| wie viel | how much | Wie viel kostet das? |
| wie lange | how long | Wie lange bleibst du? |
| welch- | which | Welches Zimmer? |
| was für ein | what kind of | Was für eine Farbe? |

---

## 11. Adjective Endings

### After definite article (der/die/das):

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominativ | -e | -e | -e | -en |
| Akkusativ | -en | -e | -e | -en |
| Dativ | -en | -en | -en | -en |

*Example: der alte Mann, den alten Mann, dem alten Mann*

### After indefinite article (ein/eine):

| Case | Masculine | Feminine | Neuter | Plural |
|------|-----------|----------|--------|--------|
| Nominativ | -er | -e | -es | -en |
| Akkusativ | -en | -e | -es | -en |
| Dativ | -en | -en | -en | -en |

*Example: ein alter Mann, einen alten Mann*

---

## 12. Common Conjunctions

| Conjunction | Meaning | Note | Example |
|-------------|---------|------|---------|
| und | and | normal word order | Peter und Helmut kommen. |
| aber | but | normal word order | Ich bin da, aber ich habe keine Zeit. |
| oder | or | normal word order | Heute oder morgen? |
| denn | because | normal word order | Ich bleibe, denn es regnet. |
| weil | because | verb → END | Ich bleibe, weil es regnet. |
| dass | that | verb → END | Ich glaube, dass er kommt. |
| wenn | when / if | verb → END | Wenn ich Zeit habe, komme ich. |

---

## 13. Imperative (Commands)

| Form | Rule | Example |
|------|------|---------|
| du (informal singular) | Verb stem (drop -en) | Komm! Mach! Fahr! |
| ihr (informal plural) | Verb stem + -t | Kommt! Macht! |
| Sie (formal) | Infinitive + Sie | Kommen Sie! Machen Sie! |

>  Irregular: sein → Sei! / Seid! / Seien Sie!

---

## 14. Negation

| Word | Use | Example |
|------|-----|---------|
| nicht | negates verbs, adjectives, adverbs | Das ist nicht richtig. |
| kein/keine | negates nouns (replaces ein/eine) | Ich habe keine Zeit. |
| nie | never | Er kommt nie pünktlich. |
| nichts | nothing | Ich kaufe nichts. |

---

## 15. Useful Fixed Phrases & Expressions

| German | English |
|--------|---------|
| Guten Morgen! | Good morning! |
| Guten Tag! | Good day! / Hello! |
| Guten Abend! | Good evening! |
| Gute Nacht! | Good night! |
| Auf Wiedersehen! | Goodbye! |
| Auf Wiederhören! | Goodbye! (on phone) |
| Tschüss! | Bye! (informal) |
| Bitte! | Please! / You're welcome! |
| Danke! / Danke schön! | Thank you! |
| Vielen Dank! | Many thanks! |
| Entschuldigung! | Excuse me! / Sorry! |
| Wie bitte? | Pardon? / Could you repeat? |
| Wie geht's? / Wie geht es Ihnen? | How are you? |
| Es geht mir gut. | I'm fine. |
| Herzlichen Glückwunsch! | Congratulations! |
| Guten Appetit! | Enjoy your meal! |
| Herzlich willkommen! | Welcome! |
| Viel Glück! | Good luck! |
| Kein Problem! | No problem! |

---
---

# PART 2 — VOCABULARY GLOSSARIES

---

## A. NOUNS WITH GENDER

### 🔵 der (Masculine)

| Noun | Plural | English |
|------|--------|---------|
| der Absender | - | sender (of letter) |
| der Anfang | -ä, e | beginning |
| der Anruf | -e | phone call |
| der Anrufbeantworter | - | answering machine |
| der Anschluss | -ü, e | connection |
| der Appetit | — | appetite |
| der Arm | -e | arm |
| der Arzt | -ä, e | doctor (male) |
| der Aufzug | -ü, e | elevator / lift |
| der Ausflug | -ü, e | excursion / day trip |
| der Ausgang | -ä, e | exit |
| der Ausweis | -e | ID card |
| der Automat | -en | machine / vending machine |
| der Bahnhof | -ä, e | train station |
| der Bahnsteig | -e | platform |
| der Balkon | -e/-s | balcony |
| der Bauch | -ä, e | stomach / belly |
| der Baum | -ä, e | tree |
| der Beamte | -n | official / civil servant |
| der Beruf | -e | profession / job |
| der Bleistift | -e | pencil |
| der Blick | -e | view / glance |
| der Brief | -e | letter |
| der Bruder | -ü | brother |
| der Buchstabe | -n | letter (of alphabet) |
| der Bus | -se | bus |
| der Chef | -s | boss / manager |
| der Computer | - | computer |
| der Dank | — | thanks / gratitude |
| der Dom | -e | cathedral |
| der Drucker | - | printer |
| der Durst | — | thirst |
| der Eingang | -ä, e | entrance |
| der Empfänger | - | recipient |
| der Erwachsene | -n | adult |
| der Fehler | - | mistake / error |
| der Film | -e | film / movie |
| der Fisch | -e | fish |
| der Flughafen | -ä | airport |
| der Freund | -e | friend (male) / boyfriend |
| der Frühling | — | spring |
| der Fuß | -ü, e | foot |
| der Fußball | — | football / soccer |
| der Garten | -ä | garden |
| der Gast | -ä, e | guest |
| der Geburtsort | -e | place of birth |
| der Geburtstag | -e | birthday |
| der Glückwunsch | -ü, e | congratulations |
| der Gruß | -ü, e | greeting |
| der Hunger | — | hunger |
| der Herd | -e | stove / cooker |
| der Herr | -en | Mr. / gentleman |
| der Hund | -e | dog |
| der Job | -s | job |
| der Junge | -n | boy |
| der Kaffee | — | coffee |
| der Koffer | - | suitcase |
| der Kollege | -n | colleague (male) |
| der Kopf | -ä, e | head |
| der Kühlschrank | -ä, e | refrigerator |
| der Kuchen | - | cake |
| der Kugelschreiber | - | ballpoint pen |
| der Kurs | -e | course |
| der Laden | -ä | shop / store |
| der Lehrer | - | teacher (male) |
| der Lkw | -s | truck |
| der Mann | -ä, er | man / husband |
| der Mensch | -en | person / human |
| der Moment | -e | moment |
| der Mond | -e | moon |
| der Morgen | - | morning |
| der Mund | -ü, er | mouth |
| der Name | -n | name |
| der Nachmittag | -e | afternoon |
| der Opa | -s | grandpa |
| der Ort | -e | place / town |
| der Partner | - | partner (male) |
| der Pass | -ä, e | passport |
| der Plan | -ä, e | plan / map |
| der Platz | -ä, e | seat / place / square |
| der Preis | -e | price / prize |
| der Prospekt | -e | brochure |
| der Raum | -ä, e | room / space |
| der Reis | — | rice |
| der Regen | — | rain |
| der Reiseführer | - | travel guide (book or person) |
| der Saft | -ä, e | juice |
| der Salat | -e | salad |
| der Schalter | - | counter / switch |
| der Schinken | - | ham |
| der Schrank | -ä, e | cupboard / wardrobe |
| der Schlüssel | - | key |
| der Schuh | -e | shoe |
| der Schüler | - | student / pupil (male) |
| der See | -n | lake |
| der Sohn | -ö, e | son |
| der Sport | — | sport |
| der Stock | -ö, e | floor / storey |
| der Student | -en | student (male, university) |
| der Tee | — | tea |
| der Teil | -e | part |
| der Termin | -e | appointment |
| der Test | -s | test |
| der Text | -e | text |
| der Tisch | -e | table |
| der Unterricht | — | lesson / class |
| der Urlaub | -e | vacation / holiday |
| der Vater | -ä | father |
| der Verein | -e | club / association |
| der Verkäufer | - | salesperson (male) |
| der Vermieter | - | landlord |
| der Verwandte | -n | relative |
| der Vorname | -n | first name |
| der Wein | -e | wine |
| der Wind | -e | wind |
| der Zoll | — | customs |
| der Zug | -ü, e | train |

---

### 🔴 die (Feminine)

| Noun | Plural | English |
|------|--------|---------|
| die Adresse | -n | address |
| die Anmeldung | -en | registration |
| die Anrede | -n | salutation / form of address |
| die Antwort | -en | answer |
| die Anzeige | -n | advertisement / notice |
| die Apotheke | -n | pharmacy |
| die Arbeit | -en | work / job |
| die Aufgabe | -n | task / exercise |
| die Auskunft | -ü, e | information |
| die Autobahn | -en | highway / motorway |
| die Bäckerei | -en | bakery |
| die Bahn | -en | train / railway |
| die Banane | -n | banana |
| die Bank | -en | bank |
| die Birne | -n | pear |
| die Bitte | -n | request |
| die Blume | -n | flower |
| die Briefmarke | -n | stamp |
| die Butter | — | butter |
| die CD | -s | CD |
| die Dame | -n | lady |
| die Disco | -s | disco / club |
| die Dusche | — | shower |
| die Ecke | -n | corner |
| die Ehefrau | -en | wife |
| die E-Mail | -s | email |
| die Entschuldigung | -en | apology / excuse |
| die Fahrkarte | -n | ticket |
| die Familie | -n | family |
| die Farbe | -n | colour |
| die Firma | Firmen | company / firm |
| die Flasche | -n | bottle |
| die Frage | -n | question |
| die Frau | -en | woman / Mrs. / wife |
| die Freizeit | — | free time |
| die Freundin | -nen | friend (female) / girlfriend |
| die Führung | -en | guided tour |
| die Geburtsjahr | -e | year of birth |
| die Geschwister | pl. | siblings |
| die Größe | -n | size |
| die Gruppe | -n | group |
| die Haltestelle | -n | (bus/tram) stop |
| die Hand | -ä, e | hand |
| die Hausaufgabe | -n | homework |
| die Hausfrau | -en | housewife |
| die Heimat | — | homeland / home |
| die Hilfe | — | help |
| die Hochzeit | -en | wedding |
| die Information | -en | information |
| die Jacke | -n | jacket |
| die Karte | -n | card / menu / map |
| die Kartoffel | -n | potato |
| die Kasse | -n | cash register / checkout |
| die Klasse | -n | class |
| die Kleidung | — | clothing |
| die Küche | -n | kitchen |
| die Lebensmittel | pl. | groceries / food |
| die Leute | pl. | people |
| die Lösung | -en | solution / answer |
| die Milch | — | milk |
| die Mitte | — | middle / centre |
| die Möbel | pl. | furniture |
| die Mutter | -ü | mother |
| die Nummer | -n | number |
| die Oma | -s | grandma |
| die Ordnung | — | order |
| die Papiere | pl. | documents / papers |
| die Partnerin | -nen | partner (female) |
| die Party | -s | party |
| die Pause | -n | break |
| die Polizei | — | police |
| die Pommes frites | pl. | French fries |
| die Post | — | post office / mail |
| die Postleitzahl | -en | postal code |
| die Praxis | Praxen | doctor's office / practice |
| die Prüfung | -en | exam |
| die Rechnung | -en | bill / invoice |
| die Reise | -n | journey / trip |
| die Reparatur | -en | repair |
| die Rezeption | -en | reception |
| die S-Bahn | -en | suburban train |
| die Schule | -n | school |
| die Schwester | -n | sister |
| die Sehenswürdigkeit | -en | tourist attraction / sight |
| die Speisekarte | -n | menu |
| die Sprache | -n | language |
| die Stadt | -ä, e | city / town |
| die Stelle | -n | position / job / place |
| die Straße | -n | street / road |
| die Straßenbahn | -en | tram |
| die Stunde | -n | hour |
| die Tasche | -n | bag / pocket |
| die Tochter | -ö | daughter |
| die Toilette | -n | toilet / restroom |
| die Tomate | -n | tomato |
| die Treppe | -n | stairs |
| die Uhr | -en | clock / watch / o'clock |
| die Unterschrift | -en | signature |
| die Vorwahl | -en | area code / dialing code |
| die Welt | -en | world |
| die Wohnung | -en | apartment / flat |
| die Zeit | -en | time |
| die Zeitung | -en | newspaper |
| die Zigarette | -n | cigarette |

---

### 🟢 das (Neuter)

| Noun | Plural | English |
|------|--------|---------|
| das Apartment | -s | apartment |
| das Auge | -n | eye |
| das Auto | -s | car |
| das Baby | -s | baby |
| das Bad | -ä, er | bathroom |
| das Bein | -e | leg |
| das Beispiel | -e | example |
| das Bett | -en | bed |
| das Bier | -e | beer |
| das Bild | -er | picture / image |
| das Brot | -e | bread |
| das Brötchen | - | bread roll |
| das Buch | -ü, er | book |
| das Café | -s | café |
| das Datum | Daten | date |
| das Dorf | -ö, er | village |
| das Doppelzimmer | - | double room |
| das Ei | -er | egg |
| das Einzelzimmer | - | single room |
| das Ergebnis | -se | result |
| das Essen | - | food / meal |
| das Fahrrad | -ä, er | bicycle |
| das Fax | -e | fax |
| das Fenster | - | window |
| das Feuer | - | fire / light (for cigarette) |
| das Fieber | — | fever / temperature |
| das Formular | -e | form |
| das Foto | -s | photo |
| das Geld | -er | money |
| das Gemüse | - | vegetables |
| das Gepäck | — | luggage |
| das Geschäft | -e | shop / business |
| das Geschenk | -e | gift / present |
| das Gespräch | -e | conversation |
| das Getränk | -e | drink / beverage |
| das Gewicht | -e | weight |
| das Glas | -ä, er | glass |
| das Gleis | -e | (railway) track / platform |
| das Glück | — | luck / happiness |
| das Haar | -e | hair |
| das Hähnchen | - | chicken |
| das Handy | -s | mobile phone |
| das Haus | -ä, er | house |
| das Hobby | -s | hobby |
| das Hotel | -s | hotel |
| das Internet | — | internet |
| das Kind | -er | child |
| das Kindergarten | -ä | kindergarten |
| das Kino | -s | cinema |
| das Kiosk | -e | kiosk / newsstand |
| das Konto | Konten | bank account |
| das Leben | - | life |
| das Licht | -er | light |
| das Lied | -er | song |
| das Lokal | -e | restaurant / pub |
| das Mädchen | - | girl |
| das Meer | -e | sea / ocean |
| das Obst | — | fruit |
| das Öl | -e | oil |
| das Papier | -e | paper |
| das Praktikum | Praktika | internship |
| das Problem | -e | problem |
| das Restaurant | -s | restaurant |
| das Sofa | -s | sofa |
| das Spiel | -e | game |
| das Studium | Studien | studies (university) |
| das Taxi | -s | taxi |
| das Thema | Themen | topic / theme |
| das Ticket | -s | ticket |
| das Wasser | - | water |
| das Wetter | — | weather |
| das Wort | -ö, er/-e | word |
| das Zimmer | - | room |

---

## B. VERBS

### Regular Verbs (Regelmäßige Verben)

| Infinitiv | English | Partizip II |
|-----------|---------|-------------|
| arbeiten | to work | gearbeitet |
| antworten | to answer | geantwortet |
| bedeuten | to mean | bedeutet |
| bezahlen | to pay | bezahlt |
| buchstabieren | to spell | buchstabiert |
| danken | to thank | gedankt |
| dauern | to last / take time | gedauert |
| drucken | to print | gedruckt |
| erklären | to explain | erklärt |
| erzählen | to tell / narrate | erzählt |
| feiern | to celebrate | gefeiert |
| fragen | to ask | gefragt |
| frühstücken | to have breakfast | gefrühstückt |
| gratulieren | to congratulate | gratuliert |
| grillen | to grill | gegrillt |
| heiraten | to marry | geheiratet |
| holen | to fetch / get | geholt |
| kaufen | to buy | gekauft |
| kochen | to cook | gekocht |
| kosten | to cost | gekostet |
| lachen | to laugh | gelacht |
| leben | to live | gelebt |
| legen | to lay / put | gelegt |
| lernen | to learn | gelernt |
| lieben | to love | geliebt |
| machen | to make / do | gemacht |
| mieten | to rent | gemietet |
| öffnen | to open | geöffnet |
| rauchen | to smoke | geraucht |
| reisen | to travel | gereist |
| reparieren | to repair | repariert |
| sagen | to say | gesagt |
| schicken | to send | geschickt |
| spielen | to play | gespielt |
| stellen | to place / put | gestellt |
| studieren | to study (univ.) | studiert |
| suchen | to look for | gesucht |
| tanzen | to dance | getanzt |
| telefonieren | to phone | telefoniert |
| trinken* | to drink | getrunken |
| verdienen | to earn | verdient |
| verkaufen | to sell | verkauft |
| vermieten | to rent out | vermietet |
| wandern | to hike | gewandert |
| warten | to wait | gewartet |
| wohnen | to live / reside | gewohnt |
| zahlen | to pay | gezahlt |
| zeigen | to show | gezeigt |

---

### Irregular Verbs (Unregelmäßige Verben) — with conjugation changes

| Infinitiv | du-form | er/sie-form | Partizip II | English |
|-----------|---------|-------------|-------------|---------|
| abfahren | fährst ab | fährt ab | abgefahren | to depart |
| abholen | holst ab | holt ab | abgeholt | to pick up |
| anfangen | fängst an | fängt an | angefangen | to begin |
| anrufen | rufst an | ruft an | angerufen | to call |
| aufstehen | stehst auf | steht auf | aufgestanden* | to get up |
| aussteigen | steigst aus | steigt aus | ausgestiegen* | to get off |
| bekommen | bekommst | bekommt | bekommen | to receive |
| bestellen | bestellst | bestellt | bestellt | to order |
| bleiben | bleibst | bleibt | geblieben* | to stay |
| bringen | bringst | bringt | gebracht | to bring |
| einsteigen | steigst ein | steigt ein | eingestiegen* | to board/get on |
| essen | isst | isst | gegessen | to eat |
| fahren | fährst | fährt | gefahren* | to drive / go |
| finden | findest | findet | gefunden | to find |
| fliegen | fliegst | fliegt | geflogen* | to fly |
| geben | gibst | gibt | gegeben | to give |
| gefallen | gefällst | gefällt | gefallen | to like / please |
| gehen | gehst | geht | gegangen* | to go / walk |
| gewinnen | gewinnst | gewinnt | gewonnen | to win |
| haben | hast | hat | gehabt | to have |
| halten | hältst | hält | gehalten | to stop / hold |
| heißen | heißt | heißt | geheißen | to be called |
| helfen | hilfst | hilft | geholfen | to help |
| kommen | kommst | kommt | gekommen* | to come |
| können | kannst | kann | gekonnt | can / to be able |
| laufen | läufst | läuft | gelaufen* | to run / walk |
| lesen | liest | liest | gelesen | to read |
| liegen | liegst | liegt | gelegen | to lie / be located |
| mitkommen | kommst mit | kommt mit | mitgekommen* | to come along |
| nehmen | nimmst | nimmt | genommen | to take |
| riechen | riechst | riecht | gerochen | to smell |
| schlafen | schläfst | schläft | geschlafen | to sleep |
| schreiben | schreibst | schreibt | geschrieben | to write |
| schwimmen | schwimmst | schwimmt | geschwommen* | to swim |
| sehen | siehst | sieht | gesehen | to see |
| sein | bist | ist | gewesen* | to be |
| sitzen | sitzt | sitzt | gesessen | to sit |
| sprechen | sprichst | spricht | gesprochen | to speak |
| stehen | stehst | steht | gestanden | to stand |
| treffen | triffst | trifft | getroffen | to meet |
| trinken | trinkst | trinkt | getrunken | to drink |
| tun | tust | tut | getan | to do |
| umziehen | ziehst um | zieht um | umgezogen* | to move (home) |
| verstehen | verstehst | versteht | verstanden | to understand |
| waschen | wäschst | wäscht | gewaschen | to wash |
| werden | wirst | wird | geworden* | to become |
| wissen | weißt | weiß | gewusst | to know (fact) |

> \* = uses sein in Perfekt

---

### Separable Verbs (Trennbare Verben) — Quick Reference

| Verb | Meaning | Partizip II |
|------|---------|-------------|
| abfahren | to depart | abgefahren |
| abgeben | to hand in | abgegeben |
| abholen | to pick up | abgeholt |
| anfangen | to start | angefangen |
| anklicken | to click on | angeklickt |
| ankommen | to arrive | angekommen |
| ankreuzen | to tick/cross | angekreuzt |
| anmachen | to turn on | angemacht |
| anmelden (sich) | to register | angemeldet |
| anrufen | to call | angerufen |
| anziehen (sich) | to get dressed | angezogen |
| aufhören | to stop | aufgehört |
| aufstehen | to get up | aufgestanden |
| aussteigen | to get off | ausgestiegen |
| ausfüllen | to fill out | ausgefüllt |
| ausziehen (sich) | to undress | ausgezogen |
| einsteigen | to board | eingestiegen |
| einkaufen | to shop | eingekauft |
| einladen | to invite | eingeladen |
| fernsehen | to watch TV | ferngesehen |
| mitkommen | to come along | mitgekommen |
| mitnehmen | to take along | mitgenommen |
| umziehen | to move (home) | umgezogen |
| vorstellen (sich) | to introduce oneself | vorgestellt |
| zuordnen | to match / assign | zugeordnet |

---

## C. ADJECTIVES (Adjektive)

| German | English | Opposite |
|--------|---------|----------|
| alt | old | jung |
| arbeitslos | unemployed | — |
| bekannt | well-known / familiar | unbekannt |
| besetzt | occupied / busy | frei |
| billig | cheap | teuer |
| böse | angry / bad | gut |
| breit | wide | schmal |
| einfach | simple / easy | schwer / kompliziert |
| fertig | finished / ready | — |
| frei | free / available | besetzt |
| fremd | foreign / unfamiliar | bekannt |
| frisch | fresh | — |
| früh | early | spät |
| gültig | valid | ungültig |
| günstig | cheap / favourable | teuer |
| glücklich | happy | unglücklich |
| groß | big / tall | klein |
| gut | good | schlecht |
| hell | bright / light | dunkel |
| hoch | high / tall | niedrig |
| international | international | — |
| jung | young | alt |
| kaputt | broken | funktionierend |
| klar | clear | unklar |
| klein | small / short | groß |
| krank | ill / sick | gesund |
| kulturell | cultural | — |
| kurz | short | lang |
| lang | long | kurz |
| langsam | slow | schnell |
| laut | loud | leise |
| ledig | single / unmarried | verheiratet |
| leicht | easy / light (weight) | schwer |
| leise | quiet | laut |
| lieb | dear / kind | — |
| lustig | funny / cheerful | langweilig |
| männlich | male | weiblich |
| müde | tired | wach |
| neu | new | alt |
| normal | normal | — |
| pünktlich | punctual / on time | unpünktlich |
| richtig | correct / right | falsch |
| ruhig | quiet / calm | laut / unruhig |
| schlecht | bad | gut |
| schnell | fast | langsam |
| schön | beautiful / nice | hässlich |
| schwer | heavy / difficult | leicht |
| selbstständig | self-employed / independent | — |
| spät | late | früh |
| teuer | expensive | billig |
| tot | dead | lebendig |
| verheiratet | married | ledig |
| voll | full | leer |
| weiblich | female | männlich |
| wenig | little / few | viel |
| wichtig | important | unwichtig |
| wunderbar | wonderful | — |
| zufrieden | satisfied | unzufrieden |

---

## D. ADVERBS (Adverbien)

### Time Adverbs

| German | English |
|--------|---------|
| heute | today |
| morgen | tomorrow |
| gestern | yesterday |
| jetzt | now |
| gleich | right away / in a moment |
| sofort | immediately |
| bald | soon |
| dann | then |
| noch | still / yet |
| schon | already |
| immer | always |
| nie | never |
| oft | often |
| manchmal | sometimes |
| meistens | mostly / usually |
| früher | before / in the past |
| später | later |
| zurzeit | currently / at the moment |
| einmal | once |

---

### Place Adverbs

| German | English |
|--------|---------|
| hier | here |
| dort / da | there |
| oben | up / upstairs |
| unten | down / downstairs |
| hinten | at the back |
| draußen | outside |
| drinnen | inside |
| links | left |
| rechts | right |
| geradeaus | straight ahead |
| daneben | next to it |
| dorthin | (to) there |

---

### Manner & Degree Adverbs

| German | English |
|--------|---------|
| sehr | very |
| auch | also / too |
| nur | only |
| leider | unfortunately |
| vielleicht | maybe / perhaps |
| gern(e) | gladly / with pleasure |
| lieber | preferably / rather |
| zusammen | together |
| automatisch | automatically |
| circa / ca. | approximately |
| besser | better |
| weit | far |
| weiter | further |

---

## E. NUMBERS, DATES & TIME

### Cardinal Numbers

| Number | German |
|--------|--------|
| 0 | null |
| 1 | eins |
| 2 | zwei |
| 3 | drei |
| 4 | vier |
| 5 | fünf |
| 6 | sechs |
| 7 | sieben |
| 8 | acht |
| 9 | neun |
| 10 | zehn |
| 11 | elf |
| 12 | zwölf |
| 13 | dreizehn |
| 20 | zwanzig |
| 21 | einundzwanzig |
| 30 | dreißig |
| 40 | vierzig |
| 50 | fünfzig |
| 100 | (ein)hundert |
| 1000 | (ein)tausend |
| 1.000.000 | eine Million |

### Ordinal Numbers (for dates)
| German | English |
|--------|---------|
| der erste (1.) | the first |
| der zweite (2.) | the second |
| der dritte (3.) | the third |
| der vierte (4.) | the fourth |
| (then add -te for 5–19, -ste for 20+) | |

### Days of the Week
| German | English |
|--------|---------|
| der Montag | Monday |
| der Dienstag | Tuesday |
| der Mittwoch | Wednesday |
| der Donnerstag | Thursday |
| der Freitag | Friday |
| der Samstag / Sonnabend | Saturday |
| der Sonntag | Sunday |
| das Wochenende | weekend |

### Months
| German | English |
|--------|---------|
| der Januar | January |
| der Februar | February |
| der März | March |
| der April | April |
| der Mai | May |
| der Juni | June |
| der Juli | July |
| der August | August |
| der September | September |
| der Oktober | October |
| der November | November |
| der Dezember | December |

### Seasons
| German | English |
|--------|---------|
| der Frühling / das Frühjahr | spring |
| der Sommer | summer |
| der Herbst | autumn / fall |
| der Winter | winter |

### Time Expressions
| German | English |
|--------|---------|
| die Sekunde | second |
| die Minute | minute |
| die Stunde | hour |
| der Tag | day |
| die Woche | week |
| das Jahr | year |
| der Morgen | morning |
| der Vormittag | late morning |
| der Mittag | midday / noon |
| der Nachmittag | afternoon |
| der Abend | evening |
| die Nacht | night |

---

## F. COLOURS & DIRECTIONS

### Colours (Farben)
| German | English |
|--------|---------|
| schwarz | black |
| weiß | white |
| grau | grey |
| rot | red |
| blau | blue |
| grün | green |
| gelb | yellow |
| braun | brown |

### Compass Directions (Himmelsrichtungen)
| German | English |
|--------|---------|
| der Norden | north |
| der Süden | south |
| der Westen | west |
| der Osten | east |

---

## G. TOPIC-BASED VOCABULARY GROUPS

### People & Family
| German | English |
|--------|---------|
| die Eltern | parents |
| der Vater / die Mutter | father / mother |
| der Bruder / die Schwester | brother / sister |
| der Sohn / die Tochter | son / daughter |
| der Großvater / die Großmutter | grandfather / grandmother |
| die Großeltern | grandparents |
| der Opa / die Oma | grandpa / grandma |
| der Mann / die Frau | husband / wife |
| der Ehemann / die Ehefrau | husband / wife (formal) |
| der Freund / die Freundin | friend / boyfriend / girlfriend |
| der Kollege / die Kollegin | colleague |
| das Kind / die Kinder | child / children |
| der Junge / das Mädchen | boy / girl |
| der Baby | baby |
| der Jugendliche | young person |
| der Erwachsene | adult |
| der Partner / die Partnerin | partner |
| der Verwandte | relative |

### Body Parts
| German | English |
|--------|---------|
| der Kopf | head |
| das Haar | hair |
| das Auge | eye |
| der Mund | mouth |
| der Arm | arm |
| die Hand | hand |
| das Bein | leg |
| der Fuß | foot |
| der Bauch | stomach / belly |

### Food & Drink
| German | English |
|--------|---------|
| das Brot | bread |
| das Brötchen | bread roll |
| die Butter | butter |
| das Ei | egg |
| das Fleisch | meat |
| der Fisch | fish |
| das Gemüse | vegetables |
| das Hähnchen | chicken |
| die Kartoffel | potato |
| der Kuchen | cake |
| die Milch | milk |
| das Obst | fruit |
| der Reis | rice |
| der Salat | salad |
| das Salz | salt |
| die Tomate | tomato |
| der Apfel | apple |
| die Banane | banana |
| die Birne | pear |
| das Öl | oil |
| das Bier | beer |
| der Kaffee | coffee |
| der Saft | juice |
| der Tee | tea |
| das Wasser | water |
| der Wein | wine |

### Home & Living
| German | English |
|--------|---------|
| das Haus | house |
| die Wohnung | apartment |
| das Zimmer | room |
| die Küche | kitchen |
| das Bad | bathroom |
| das Schlafzimmer | bedroom |
| der Balkon | balcony |
| der Garten | garden |
| das Bett | bed |
| der Schrank | cupboard |
| das Sofa | sofa |
| der Tisch | table |
| der Stuhl | chair |
| die Treppe | stairs |
| der Aufzug | elevator |
| der Herd | stove |
| der Kühlschrank | refrigerator |
| die Miete | rent |
| der Vermieter | landlord |

### Transport & Travel
| German | English |
|--------|---------|
| das Auto | car |
| die Bahn | train / railway |
| der Bus | bus |
| das Fahrrad | bicycle |
| das Flugzeug | airplane |
| das Taxi | taxi |
| die Straßenbahn | tram |
| die S-Bahn | suburban train |
| der Zug | train |
| der Bahnhof | train station |
| der Bahnsteig | platform |
| die Fahrkarte | ticket |
| der Flughafen | airport |
| die Autobahn | motorway |
| die Haltestelle | stop |
| die Abfahrt | departure |
| die Ankunft | arrival |
| der Ausflug | trip / excursion |
| das Gepäck | luggage |
| der Koffer | suitcase |
| der Pass | passport |
| das Ticket | ticket |
| das Reisebüro | travel agency |

### Health
| German | English |
|--------|---------|
| der Arzt / die Ärztin | doctor |
| krank | ill / sick |
| das Fieber | fever |
| der Doktor | doctor |
| die Praxis | doctor's office |
| der Termin | appointment |
| weh tun | to hurt |
| der Kopf tut weh | my head hurts |

### Shopping & Money
| German | English |
|--------|---------|
| das Geschäft | shop |
| der Laden | store |
| die Kasse | cash register |
| der Preis | price |
| das Geld | money |
| die Rechnung | bill |
| kaufen | to buy |
| bezahlen | to pay |
| billig | cheap |
| teuer | expensive |
| bar | cash |
| die Karte | (credit/debit) card |
| das Angebot | offer / deal |
| günstig | cheap / good value |

### Communication & Technology
| German | English |
|--------|---------|
| das Handy | mobile phone |
| das Telefon | telephone |
| die E-Mail | email |
| das Fax | fax |
| das Internet | internet |
| der Computer | computer |
| der Drucker | printer |
| anrufen | to call |
| telefonieren | to telephone |
| schicken | to send |
| die Vorwahl | area code |
| der Anrufbeantworter | answering machine |

---

*Source: Goethe-Institut, Wortliste zu Start Deutsch 1 (A1), official publication.*
